...
import forumPalestra from "@/assets/optimized/forum-palestra.webp";
...
type FounderFernandoProps = {
  fernandoAvatar: OptimizedImage;
  globonews: OptimizedImage;
  pecege: OptimizedImage;
};

const FounderFernando = ({ fernandoAvatar, globonews, pecege }: FounderFernandoProps) => {
  const [zoomIndex, setZoomIndex] = useState<number | null>(null);
  const images = [
    { src: assetSrc(palestraEvento), optimized: null, alt: "Fernando Godoy na GloboNews", position: "center" },
    { src: assetSrc(forumPalestra), optimized: null, alt: "Fernando Godoy palestrando", position: "center" },
    { src: globonews.fallbackSrc, optimized: globonews, alt: "Fernando Godoy no programa Conta Corrente - GloboNews", position: "center" },
    { src: assetSrc(palco), optimized: null, alt: "Fernando Godoy em grande palco", position: "center" },
    { src: pecege.fallbackSrc, optimized: pecege, alt: "Fernando Godoy palestrando no PECEGE", position: "top" },
    { src: assetSrc(nexialista), optimized: null, alt: "Fernando Godoy - O Nexialista É Antifrágil", position: "center" },
    { src: assetSrc(workshop), optimized: null, alt: "Fernando Godoy em workshop", position: "center" },
    { src: assetSrc(aifirst), optimized: null, alt: "Fernando Godoy - AI First", position: "top" },
    { src: assetSrc(forumPainel), optimized: null, alt: "Fernando Godoy no Painel de IA - Fórum Negócios", position: "center" },
    { src: assetSrc(forumChatgpt), optimized: null, alt: "Fernando Godoy no Fórum Negócios Brasil", position: "center" },
    { src: assetSrc(podcast), optimized: null, alt: "Fernando Godoy no InovaMente Cast", position: "center" },
    { src: assetSrc(gala), optimized: null, alt: "Fernando Godoy em evento Gala Business", position: "top" },
    { src: assetSrc(primocast), optimized: null, alt: "Fernando Godoy no PrimoCast", position: "center" },
  ];
                    ...
                    {images.map((img, i) => (
                      <CarouselItem key={i}>
                        ...
                        >
                          {img.optimized ? (
                            <picture>
                              <source type="image/avif" srcSet={img.optimized.avifSrcSet} sizes={img.optimized.sizes} />
                              <source type="image/webp" srcSet={img.optimized.webpSrcSet} sizes={img.optimized.sizes} />
                              <img
                                src={img.optimized.fallbackSrc}
                                alt={img.alt}
              ...
              <div className="space-y-6">
                <div className="flex items-center gap-4">
                  <div className="w-20 h-20 rounded-full overflow-hidden">
                    <picture>
                      <source type="image/avif" srcSet={fernandoAvatar.avifSrcSet} sizes={fernandoAvatar.sizes} />
                      <source type="image/webp" srcSet={fernandoAvatar.webpSrcSet} sizes={fernandoAvatar.sizes} />
                      <img
                        src={fernandoAvatar.fallbackSrc}
                        alt="Fernando Godoy"
                        width={fernandoAvatar.width}
                        height={fernandoAvatar.height}
                        loading="eager"
                        fetchPriority="high"
                        decoding="async"
